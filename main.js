...
const PURPLE = { r: 0x8a, g: 0x2b, b: 0xe2 }; // Фиолетовый
const YELLOW = { r: 0xff, g: 0xd7, b: 0x00 }; // Жёлтый
const RED =    { r: 0xff, g: 0x45, b: 0x00 }; // Красный
const CYAN =   { r: 0x00, g: 0xff, b: 0xff }; // Бирюзовый
const LIME =   { r: 0x32, g: 0xcd, b: 0x32 }; // Лаймовый

const allColors = [BLUE, GREEN, PINK, ORANGE, PURPLE, YELLOW, RED, CYAN, LIME];
...
const getTarget = (() => {
	...
	return function getTarget() {
		...
		// Target Parameters
		// --------------------------------
		let color = pickOne([BLUE, GREEN, PINK, ORANGE, PURPLE, YELLOW, RED, CYAN, LIME]);
		let wireframe = false;
		let health = 1;
		let maxHealth = 3;
...
function spawnSpecialBonus() {
    const bonus = getTarget();
    bonus.color = ORANGE; // Цвет бонуса
    bonus.x = Math.random() * 400 - 200; // Случайное место появления
    bonus.y = -100; // Начальная позиция
    bonus.yD = 5; // Скорость движения вниз
    targets.push(bonus); // Добавляем бонус в список целей
}
...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `Очко: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `Разбитые кубики: ${state.game.cubeCount}`;
}
...
function showLevelNotification(text) {
    ...
    levelText.textContent = text; // Устанавливаем текст уведомления
